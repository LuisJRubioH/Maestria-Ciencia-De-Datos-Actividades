...
La Figura \@ref(fig:box-numerica-response) presenta los boxplots comparativos por grupo. La Tabla \@ref(tab:comp-numeric-response) resume los estadísticos descriptivos por grupo y los p-valores del test de Mann-Whitney para la comparación de variables numéricas por respuesta a la campaña. 

<div style="overflow-x: auto; width: 100%;">
```{r comp-numeric-response, echo=FALSE}
...
**NumWebVisitsMonth:**  (p = 0.741, r = 0.012, efecto despreciable): No se detectaron diferencias significativas entre grupos. La frecuencia de visitas al sitio web no discrimina entre aceptantes y no aceptantes en esta muestra, lo que sugiere que la interacción digital pasiva (visitar la web) no se traduce en mayor propensión a aceptar ofertas de marketing directo..
...
**Recency:** (p = 0.000, r = −0.334, efecto moderado): Los clientes que aceptaron la campaña realizaron su última compra más recientemente (mediana 29 días) frente a los que rechazaron (mediana 52 días). El signo negativo del efecto indica que menor recencia se asocia con mayor aceptación. Esto es coherente: clientes con actividad de compra reciente están más vinculados con la marca y son más receptivos a nuevas ofertas. Es la tercera variable más discriminante del análisis, después de MntTotal y Recency misma empatando con Customer_Days en magnitud.
...
La recencia de compra (Recency) presenta asociación estadísticamente significativa (p < 0.001) con un tamaño del efecto moderado (r = −0.334), como se reporta en la Tabla \@ref(tab:effect-sizes-num). Los clientes que aceptaron la campaña realizaron su última compra más recientemente (mediana de 29 días frente a 52 días, Tabla \@ref(tab:comp-numeric-response)), patrón que se visualiza en la Figura \@ref(fig:box-numerica-response). Esto sugiere que la actividad transaccional reciente refleja un mayor vínculo con la marca y una mayor receptividad a nuevas ofertas. De forma complementaria, la antigüedad del cliente (Customer_Days) muestra un efecto moderado similar (r = 0.307, p < 0.001, Tabla \@ref(tab:effect-sizes-num)), indicando que clientes con mayor permanencia tienden a aceptar más la oferta. Ambas variables capturan dimensiones temporales distintas pero complementarias: la primera mide actividad reciente y la segunda mide lealtad acumulada.

La frecuencia de visitas web mensuales (NumWebVisitsMonth) no presentó diferencias significativas entre grupos (p = 0.741, Tabla \@ref(tab:comp-numeric-response)), con un tamaño del efecto despreciable (r = 0.012, Tabla \@ref(tab:effect-sizes-num)). La interacción digital pasiva, medida como número de visitas al sitio web, no constituye un factor discriminante en esta muestra. Esto indica que visitar el sitio web no se traduce necesariamente en mayor propensión a aceptar ofertas de marketing directo.

Las variables demográficas clásicas (edad, nivel educativo, estado civil) no presentan capacidad discriminante significativa en esta muestra (Tabla \@ref(tab:comp-numeric-response) y Tabla \@ref(tab:chi-cuadrado)). Esto sugiere que el perfil del cliente respondiente se define más por su patrón de consumo y su actividad transaccional reciente que por sus características sociodemográficas, hallazgo consistente con la literatura en marketing directo donde el comportamiento previo supera a la demografía como predictor de respuesta.

La estructura del hogar (Kidhome) muestra una asociación débil pero significativa (Tabla \@ref(tab:chi-cuadrado) y Tabla \@ref(tab:prop-kidhome)), como se visualiza en la Figura \@ref(fig:barplot-categorico), posiblemente mediada por la disponibilidad de ingreso discrecional. Esta variable podría aportar capacidad explicativa complementaria en un modelo multivariante que controle simultáneamente por ingreso.
