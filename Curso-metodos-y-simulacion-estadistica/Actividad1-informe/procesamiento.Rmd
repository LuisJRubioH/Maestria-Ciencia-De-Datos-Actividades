...
```
Se identificaron **`r total_duplicados`** registros duplicados (representando el **`r round(total_duplicados/nrow(datos_raw)*100, 2)`%** del total). Estos corresponden a clientes con información idéntica en todas las variables. La presencia de duplicados introduce sesgo en los estimadores de tendencia central y dispersión, inflando artificialmente el tamaño muestral y violando el supuesto de independencia. Según Barnett y Lewis (1994), duplicados exactos deben eliminarse antes del análisis para evitar sobreponderación de observaciones particulares. 
...
# Función para calculae¿r estadisticps descriptivos
...
knitr::kable(
  tabla_estadisticos,
  caption = "Estadísticos descriptivos y límites IQR",
  col.names = c("Variable", "Mín", "Media","std", "Q1", "Mediana","Q3", 
                "Máx"),
...
La Tabla \@ref(tab:total-porcentaje-atipicos) muestra la detección de valores atípicos mediante el método **IQR** (rango intercuartílico Tukey, 1977) el cual es una medida de dispersión robusta que cuantifica la amplitud del $50 \%$ central de la distribución, con límites definidos como:
...
Un valor $x$ es atípico si:

$$𝑥 <  L_{inf} = Q_1 - (1.5 \cdot IQR) \quad \text{o} \quad x > L_{sup} = Q_3 + (1.5 \cdot IQR)$$
...
knitr::kable(
  ...
  caption = "Resumen de total valores atípicos detectados",
  col.names = c("Variable", "lim_inf", "lim_sup", "IQR","N° atípicos", "% atípicos"),
  align = c("l", "r", "r", "r", "r", "r"),
  row.names = FALSE
)
```
...
```{r boxplotcontinuas, echo=FALSE, fig.cap="Distribución de variables numéricas continuas", echo=FALSE}
...
ggplot(datos_largo, aes(x = Variable, y = Valor, fill = Variable)) +
  ...
  labs(
    title = "Distribución de variables monetarias continuas",
    x = "",
    y = "Valor"
  )
```
...
- **Income:** Distribución asimétrica positiva (sesgo derecho) con mediana ≈ $52,000. Atípicos superiores representan clientes de muy alto ingreso (segmento premium).
- **MntWines:** Mayor variabilidad en gasto. Categoría con mayor consumo promedio.
- **MntFruits, MntFish, MntSweet, MntGold:** Cajas comprimidas cerca de cero. Mayoría gasta poco; atípicos superiores indican compradores especializados.
- **MntTotal y MntRegularProds:** Alta correlación esperada. Dispersión amplia refleja heterogeneidad en comportamiento de compra.


#### Estudio de las variables discretas

```{r boxplot-discretas, fig.cap="Boxplots de variables numéricas discretas", echo=FALSE}
...
ggplot(datos_largo_disc, aes(x = "", y = Valor, fill = Variable)) +
  ...
  labs(
    title = "Distribución de variables numéricas discretas",
    x = "",
    y = "Valor"
  )
```

La Figura \@ref(fig:boxplot-discretas) muestra la distribución de variables numéricas discretas. De donde podemos resaltar

- **Age:** Presencia de atípicos superiores (>80 años) requiere validación.
- **NumStorePurchases:** Canal preferido (mediana ≈ 5-6), distribución simétrica.
- **NumWebPurchases:** Adopción moderada del canal digital.
- **Kidhome/Teenhome:** Mayoría sin dependientes (mediana = 0).
- **Customer_Days:** Alta antigüedad promedio 2511 días ≈ 6.9 años, indicando lealtad.



#### Estudio de variables binarias

La Figura \@ref(fig:barras-binarias) muestra la distribución de frecuencias para variables categóricas binarias.

```{r barras-binarias, fig.cap="Distribución de variables binarias", fig.width=12, fig.height=16, , echo=FALSE}
...
- **Campañas de marketing:** Tasa de aceptación menor al **15\%** en todas las campañas, sugiriendo problema estructural de efectividad.
- **Perfil demográfico:** Predominio de clientes casados entre el **35\%** y *40\%*, con educación superior aproximadamente del **70\%**.
- **Quejas:** Frecuencia muy baja menos del **5\%**, indicando satisfacción general.
- **Desbalance de clases:** Variable objetivo (Response) presenta desbalance **85:15**, implicación para modelado predictivo futuro.



## Análisis de datos atípicos

Los valores atípicos observados en variables de gasto como por ejemplo, *MntTotal *, *MntWines* y otras variables monetarias serán conservados, ya que representan el segmento *premium* de clientes de alto valor. Según Rousseeuw y Hubert (2011), valores atípicos en el dominio natural de la variable deben conservarse cuando reflejan heterogeneidad real del fenómeno estudiado. Eliminarlos sesgaría la estimación del potencial de ingresos y reduciría artificialmente la varianza poblacional.

Por otro lado, los valores extremos en variables de comportamiento como *NumWebVisitsMonth* o *NumDealsPurchases* también se mantendrán, dado que reflejan patrones auténticos de uso intensivo o baja interacción, los cuales son relevantes para la segmentación y el modelado predictivo. En síntesis, se eliminarán únicamente observaciones inconsistentes desde el punto de vista lógico y/o biológico, mientras que se conservaron aquellas que, aunque extremas, representan comportamientos reales y económicamente significativos dentro de la población analizada.

 
#### Revisión en detalle de inconsistencias 
Se detectaron inconsistencias estructurales en las variables **MntRegularProds**, **Age** y **marital_Divorced** comos e muestra en la Tabla \@ref(tab:tabla-inconsistencias). En el caso de **MntRegularProds** se identificaron *valores negativos*, los cuales violan el dominio teórico de la variable (monto ≥ 0). En **Age** se observaron registros con valor **240**, imposible desde el punto de vista biológico.Límite superior razonable: 110 años (récord mundial: 122 años, Jeanne Calment). Asimismo, **marital_Divorced** presentó valores distintos de **0** y **1**, lo que viola la naturaleza binaria.
...
knitr::kable(
  ...
  col.names = c("Variable", "Criterio de validez", "N° inválidos", "Valores detectados"),
  align = c("l", "l", "c", "l")
)
```
...
Según Osborne y Overbay (2004), inconsistencias que violan restricciones del dominio deben eliminarse por completo, ya que no admiten corrección sin introducir mayor incertidumbre.
...
Tras la eliminación de registros duplicados, la base quedó compuesta por *2.041* registros.
Se eliminaron **`r n_eliminados`** registros (**`r pct_eliminados`%** del total de los *2.041*), que presentaban inconsistencias estructurales: valores negativos en MntRegularProds, edades imposibles (*240 año*s) y codificaciones inválidas en marital_Divorced (valor *100*). Dado que estas observaciones violan el dominio teórico de las variables y representan una proporción marginal de la muestra, se optó por su eliminación completa. Resultando en **`r nrow(datos_limpios)`** observaciones válidas.
...
knitr::kable(
  comparacion,
  digits = 2,
  caption = "Comparación de indicadores descriptivos antes y después del tratamiento",
  align = c("l", "l", "r", "r", "r", "r", "r")
)
```


Para la variable **MntRegularProds**, según la Tabla \@ref(tab:mntregula-Age-antes-despues), - Media aumenta ligeramente de **520.59** a **522.72**, es decir, un incremento de **2.13** unidades (**≈ 0.4\%**). En términos relativos, el cambio es despreciable. 
- La mediana pasa de **288.0** a **290.5**, lo que confirma que la posición central robusta prácticamente no se altera. 
- La desviación estándar se mantiene esencialmente constante **555.80** frente a **555.82** (**≈0.04\%**).
- Los cuartiles $Q_1$ y $Q_3$ presentan variaciones mínimas.

Esto indica que la dispersión intercuartílica permanece estable. La Figura \@ref(fig:comp-boxplots) corrobora visualmente este comportamiento: los boxplots antes y después del tratamiento presentan estructura y rango intercuartílico prácticamente idénticos.

En consecuencia, la eliminación de los tres valores negativos no modificó de manera sustancial los indicadores de tendencia central ni de dispersión. Dichas observaciones, aunque inconsistentes desde el punto de vista lógico, no ejercían influencia relevante sobre la estructura estadística global de la variable.


Para la variable **Age**, la Tabla \@ref(tab:mntregula-Age-antes-despues) muestran que
- La media disminuye de **51.35** a **51.17** (**≈0.35\%**). Esto indica que los registros con edad 240 inflaban ligeramente el promedio. 

- La mediana permanece exactamente en **50**, lo que confirma que el valor extremo no afectaba la posición central robusta.

- La desviación estándar disminuye de **13.04** a **11.65**, lo que representa una reducción relativa aproximada del **10.6\%**. Este cambio evidencia que el registro con edad 240 generaba una sobreestimación apreciable de la variabilidad. 

- Los cuartiles $Q_1$ y $Q_3$ no presentan modificaciones, por lo que la dispersión intercuartílica se mantiene estable.

La Figura \@ref(fig:comp-boxplots) respalda este análisis: el boxplot posterior al tratamiento muestra una reducción clara en la amplitud total, mientras que la caja central conserva su posición y tamaño.

Por tanto, la eliminación del valor atípico produjo una disminución significativa en la desviación estándar, corrigiendo la sobreestimación de la variabilidad, pero sin alterar la estructura central de la distribución.

```{r comp-boxplots, fig.width=12, fig.height=5, fig.cap="Comparación de distribuciones antes (rojo) y después (azul) del tratamiento de inconsistencias.", echo=FALSE}
# MntRegularProds
p1 <- ggplot() +
  geom_boxplot(data = datos, aes(x = "Antes", y = MntRegularProds), 
               fill = "#E74C3C", alpha = 0.8) +
  geom_boxplot(data = datos_limpios, aes(x = "Después", y = MntRegularProds), 
...
p2 <- ggplot() +
  geom_boxplot(data = datos, aes(x = "Antes", y = Age), 
               fill = "#E74C3C", alpha = 0.8) +
  geom_boxplot(data = datos_limpios, aes(x = "Después", y = Age), 
               fill = "#3498DB", alpha = 0.8) +
  labs(title = "Age", y = "Edad (años)", x = "") +
  theme_minimal()
...
Para una variable binaria como marital_Divorced, lo adecuado es una tabla de distribución de frecuencias absoluta y relativa, incluyendo proporciones.
...
knitr::kable(
  tabla_marital,
  caption = "Distribución de la variable binaria marital_Divorced antes del tratamiento.",
  label = "marital-divorced-antes",
  col.names = c("Categoría (0 = No divorciado, 1 = Divorciado, 100 = Inconsistencia)",
                "Frecuencia",
                "Porcentaje (%)"),
  align = c("c", "c", "c")
)
...
knitr::kable(
  tabla_marital,
  caption = "Distribución de la variable binaria marital_Divorced después del tratamiento.",
  label = "marital-divorced-despues",
  col.names = c("Categoría (0 = No divorciado, 1 = Divorciado)",
                "Frecuencia",
                "Porcentaje (%)"),
  align = c("c", "c", "c")
)
...
knitr::kable(
  tabla_marital_comp,
  caption = "Distribución de marital_Divorced antes y después del tratamiento",
  col.names = c("Momento", "Categoría", "Frecuencia", "Porcentaje (%)"),
  align = c("l", "c", "r", "r")
)
```

```{r barras-marital-divorced-comparacion, fig.width=12, fig.height=5, fig.cap="Comparación de la distribución de marital_Divorced antes y después del tratamiento.", echo=FALSE}
...
Antes del tratamiento se identificaron **6** registros con valor **100** como se observa en la Tabla \@ref(tab:marital-divorced-antes) , correspondiente al **0.29\%** del total de las **2041** observaciones. Tras su eliminación, la estructura porcentual de la variable prácticamente no se modificó como se puede observar en la Tabla \@ref(tab:marital-divorced-despues) , lo cual indica que la inconsistencia no afectaba sustancialmente la distribución de la variable, lo cual es más evidente en la gráfica de la figura \@ref(fig:barras-marital-divorced-comparacion).



### Validación y evaluación de la depuración

La principal preocupación al eliminar registros es introducir un sesgo, alterando la estructura subyacente de los datos. Vamos a resposnder a la sigyuiente pregunta

¿La eliminación de duplicados e inconsistentes ha sido una decisión metodológicamente sólida y, por lo tanto, podemos proceder con confianza al tratamiento de los valores faltantes en el conjunto de datos de 2030 registros?
...
knitr::kable(
  tabla_tamano,
  caption = "Comparación del tamaño muestral antes y después de eliminar inconsistencias",
...
knitr::kable(
  tabla_comparacion,
  caption = "Comparación de estadísticos descriptivos antes y después de la depuración",
  align = "l",
  digits = 3
)
...
knitr::kable(
  tabla_ks,
  caption = "Prueba de Kolmogorov–Smirnov para igualdad de distribuciones",
  align = "c"
)
...
knitr::kable(
  tabla_cor,
  caption = "Diferencia global entre matrices de correlación (Norma de Frobenius)",
  align = "c"
)
...
En la Tabla \@ref(tab:esta-desc-original-limpios) es crucial observar que las medias, desviaciones estándar y medianas de todas las variables permanecen prácticamente idénticas. Por ejemplo, la media de **Income** pasa de **516,535** a **517,963**, un cambio de apenas **0.27\%**. La desviación estándar también se mantiene estable. En términos estadísticos, esto indica que la localización y la escala de las distribuciones no se han visto afectadas. La estructura central de los datos es la misma.

La Tabla \@ref(tab:prueba-KS) muestra la evidencia más sólida. La **prueba KS** es una herramienta no paramétrica poderosa para comparar distribuciones completas. Los **estadísticos D** (que miden la distancia máxima entre las distribuciones acumuladas) son extremadamente bajos por ejemplo, **0.0065** para **Income**, y los **p-values** son exactamente **1**, lo que cnfirma no existe evidencia estadística que sugiera que las distribuciones, antes y después de la limpieza, sean diferentes. Son, a efectos prácticos, idénticas.


En la Tabla \@ref(tab:prueba-Frobenius) la norma de Frobenius cuantifica la diferencia global entre dos matrices. Un valor de **0.0558** es extraordinariamente bajo. Esto significa que las relaciones lineales entre las variables se han mantenido casi inalteradas. La estructura de correlación, que es la "anatomía" de las relaciones entre variables, sigue siendo la misma.

En resumen, la eliminación del **8.56%** de los datos (190 registros, Tabla \@ref(tab:muestra-original-limpios)) ha sido un proceso de limpieza efectivo. No ha introducido sesgo ni ha alterado las propiedades estadísticas fundamentales del conjunto de datos. Hemos eliminado ruido (duplicados e inconsistencias) conservando la esencia de la información, lo que valida metodológicamente el uso del conjunto depurado de 
...
La  Figura \@ref(fig:fig-datos-faltantes) se presenta un resumen de las variables ordenadas según el número de datos faltantes.
...
La Figura \@ref(fig:vis-missing) muestra el patrón espacial de datos faltantes.
...
La revisión conjunta de las Figuras \@ref(fig:fig-datos-faltantes) y \@ref(fig:vis-missing), junto con las Tablas \@ref(tab:tabla-nulos-original), \@ref(tab:tabla-nulos-sin-duplicados) y \@ref(tab:tabla-nulos), confirma que el problema de datos faltantes es reducido y está concentrado exclusivamente en las variables 
**Income** y **MntWines**. En el dataset original se observan 68 valores faltantes en **Income` (3.06%)** y **20** en **MntWines (0.90%)**. Tras eliminar duplicados, las proporciones se mantienen prácticamente inalteradas (**3.33\%** y **0.98\%**, respectivamente). Finalmente, en el conjunto depurado sin duplicados ni inconsistencias, **Income** presenta **66** valores faltantes (**3.25\%**) y **MntWines** presenta **20** (**0.99\%**). 

Las Figuras \@ref(fig:fig-datos-faltantes) y \@ref(fig:vis-missing) muestran que el resto de variables no presentan ausencia de datos y que los faltantes aparecen de forma esporádica y dispersa, sin aparente evidencia de patrones estructurados ni bloques sistemáticos de coausencia. Dado que todas las proporciones están por debajo del umbral del **5\%** señalado en la literatura especializada  (Little y Rubin, 2002), el nivel de datos faltantes puede considerarse bajo y su impacto potencial sobre los análisis posteriores es limitado y manejable mediante procedimientos estándar de tratamiento de valores ausentes.


## Tratamiento de Datos Faltantes

### Evaluación del mecanismo generador de datos faltantes

Para evaluar si los datos faltan completamente al azar (MCAR), se aplica el test de Little (1988) considerando un conjunto amplio de variables que podrían estar relacionadas con la probabilidad de missing. Se incluyen: ingresos, gasto en vinos, edad, número de hijos, antigüedad, frecuencia de compras y variables demográficas codificadas como factores.

```{r test-mca, echo=FALSE}
## Reconstruir variables categóricas originales a partir de dummies
...
knitr::kable(
  data.frame(
    Estadístico = test_little$statistic,
    gl = test_little$df,
...
Dado que el p-valor = `r round(test_little$p.value, 4)` < 0.05, se rechaza $H_{0}$. En consecuencia, los datos no siguen un mecanismo completamente aleatorio (**MCAR**). No obstante, dado el bajo porcentaje de ausencia (**<5%**) y la ausencia de evidencia estructural de **MNAR (Missing Not At Random)**, se asume razonablemente un mecanismo **MAR (Missing At Random)** para efectos de imputación múltiple.


Bajo **MAR**, la eliminación por lista completa puede inducir sesgo y pérdida innecesaria de eficiencia. Por ello se emplea imputación múltiple según Rubin (1987), que:

- Preserva la estructura multivariada.

- Incorpora la incertidumbre de imputación.

- Mantiene la variabilidad poblacional.



Se excluyen variables que constituyen combinaciones lineales exactas (por ejemplo, **MntTotal**, **MntRegularProds** y **AcceptedCmpOverall**), ya que generan multicolinealidad perfecta y matrices singulares en el algoritmo de imputación (criterio clásico de diagnóstico estructural). Las variables derivadas se recalculan posteriormente.

```r
# Colinealidad perfecta (VIF = ∞)
...
Se comparan tres métodos: **PMM (Predictive Mean Matching)** con regularización ridge, **CART (Classification and Regression Trees)** y **Regresión ridge**
...
Para elegir el método que mejor preserva la estructura de los datos observados, se evalúan:

1. Estabilidad de las imputaciones: desviación estándar de las medias imputadas entre las 5 réplicas.

2. Preservación de momentos: comparación de medias y varianzas de las variables imputadas con las observadas (listwise deletion).

3. Preservación de correlaciones: comparación de la matriz de correlación de las variables clave (Income, MntWines, Age, MntTotal) con la observada.

Los resultados se presentan en la Tabla \@ref(tab:comp-metd-imputacion).

<div style="overflow-x: auto; width: 100%;">
```{r comp-metd-imputacion, echo=FALSE}
# Función para extraer estadísticos de un objeto mice
...
Adicionalmente en la Tabla \@ref(tab:tabla-cor), se comparan las correlaciones de Pearson entre Income, MntWines y Age para cada método (promedio de las 5 imputaciones) frente a la correlación observada.
...
# Función para correlaciones promedio de un método
...
knitr::kable(tabla_cor, digits = 3,
             caption = "Comparación de correlaciones promedio (5 imputaciones) con las observadas.")
```
</div>

Basado en la menor diferencia relativa de medias y desviaciones, así como la preservación de correlaciones, se selecciona **PMM** con ridge como método final (van Buuren, 2018, Flexible Imputation of Missing Data). Este método presenta la mayor estabilidad (baja desviación entre imputaciones) y reproduce fielmente la estructura correlacional.


Se utiliza la primera imputación de *PMM* para el análisis descriptivo, previa verificación de que las 5 imputaciones producen resultados muy similares (Tabla \@ref(tab:consistencia-imputaciones).
...
Para la validación estructural post-imputación, se comparan densidades antes y después de imputar como se ne la Figura \@ref(fig:densidad-compa).
...
p1 <- ggplot() +
  ...
  theme_minimal() +
  theme(
    plot.title = element_text(size = 10),   # reduce tamaño para que no se corte
...
Las curvas muestran superposición casi completa, sin distorsión visible. Finalmente, se analiza la matriz de correlaciones en la Figura \@ref(fig:matriz-cor).

```{r matriz-cor, echo=FALSE, fig.cap="Matriz de correlaciones de variables numéricas."}
...
corrplot(cor_matrix, method = "color", type = "upper", 
         ...
         number.cex = 0.7,            # Tamaño del texto
         number.digits = 2,           # Decimales
         title = "Matriz de correlaciones de variables numéricas",
         mar = c(0,0,2,0))
```

Las relaciones esperadas (por ejemplo, alta correlación entre Income y MntWines) se mantienen, lo que confirma preservación estructural. También, correlaciones moderadas entre distintos tipos de gasto, indicando cierto patrón de consumo conjunto. Además, se rsalta que la variable Recency presenta correlaciones bajas con el resto, sugiriendo independencia.
